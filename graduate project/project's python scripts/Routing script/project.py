...
def create_vlans1():
    ...
    print(f"entry_widget2_list length: {len(entry_widget2_list)}")
    print(f"entry_widget2_list: {entry_widget2_list}")
    ...
    if ( check_integers(vlan_name) or not vlan_name ) and not check_integers(vlan_id) :
        ...
        vlan_id=[]
        vlan_name=[]
        print(not check_integers(vlan_id))
...
def create_vlans2():
    ...
    print(vlan_id) 
    # if not duplicate_id and not duplicate_name and not check_integers(vlan_name) and not (check_integers(vlan_name) or not check_integers(vlan_id)) :
    print(not duplicate_nameint and not duplicate_idint and not (check_integers(vlan_name) or not check_integers(vlan_id)))
...
def append_interface(vlan, switch, new_interface,choices_int):
    ...
    if vlan in choices_int:
        ...
        if switch in choices_int[vlan]:
            ...
            print(f"Added '{new_interface}' to {switch} in {vlan}")
        else:
            choices_int[vlan][switch] = [[new_interface]]
            print(f"Switch {switch} not found in {vlan}.")
    else:
        choices_int[vlan] = {switch: [[new_interface]]}
...
def window_security():
    ...
    if dhcp_vlan:
        ...
        combo1_dhcp = ctk.CTkOptionMenu(drop_frame_dhcp, values=dhcp_vlan)
        ...
        dhcp_label2 = ctk.CTkLabel(master=drop_frame_dhcp, text="No VLANs yet")
        dhcp_label2.grid(row=0, column=0, padx=10, pady=5)
